...
export const fetchConstraintActions = async (
  constraintId: number,
...
export const createConstraintAction = async (
  constraintId: number,
  data: Partial<ConstraintAction>,
): Promise<ConstraintAction> => {
  const res = await apiFetch(`/toc/constraints/${constraintId}/actions`, {
    method: "POST",
    body: data,
...
export const fetchThroughputMetrics = async (params?: {
  period_start?: string;
  period_end?: string;
...
export const calculateThroughput = async (
  data: any,
): Promise<ThroughputMetrics> => {
