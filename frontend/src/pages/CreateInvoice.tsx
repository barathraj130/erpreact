...
const CreateInvoice: React.FC = () => {
  ...
  const [invoiceNo, setInvoiceNo] = useState(""); // custom number typed by user (empty = auto)
  const [isAutoMode, setIsAutoMode] = useState(true); // true = auto-generate on save
  const [nextInvoiceNo, setNextInvoiceNo] = useState<string>(""); // preview from backend
  ...
  useEffect(() => {
    const fetchNextNumber = async () => {
      try {
        const purpose = (invoiceType as string) === 'name_only' ? 'name_only' : 'real';
        const r = await apiFetch(`/invoice/preview-number?type=${invoiceType}&purpose=${purpose}`);
        const data = await r.json();
        if (data.next_number) setNextInvoiceNo(data.next_number);
      } catch {
        setNextInvoiceNo("");
      }
    };
    fetchNextNumber();
    // When bill type changes, reset to auto mode
    setIsAutoMode(true);
    setInvoiceNo("");
  }, [invoiceType]);
            ...
            <div className="ci-row-3">
              <div className="ci-field">
                ...
                <div style={{
                  display: "flex",
                  alignItems: "center",
                  border: `1.5px solid ${isAutoMode ? "#86efac" : "#e5e7eb"}`,
                  borderRadius: "8px",
                  overflow: "hidden",
                  background: "white",
                  ...
                  <input
                    type="text"
                    value={isAutoMode ? "" : invoiceNo}
                    onChange={(e) => {
                      setInvoiceNo(e.target.value);
                      setIsAutoMode(false);
                    }}
                    placeholder={nextInvoiceNo || "Auto-generating…"}
                    style={{
                      flex: 1,
                      padding: "10px 12px",
                      border: "none",
                      ...
                      fontSize: "14px",
                      fontWeight: 600,
                      color: isAutoMode ? "#9ca3af" : "#111827",
                      background: "transparent",
                    }}
                  />
                  <button
                    type="button"
                    onClick={() => { setIsAutoMode(true); setInvoiceNo(""); }}
                    style={{
                      padding: "6px 10px",
                      margin: "4px",
                      background: isAutoMode ? "#dcfce7" : "#f3f4f6",
                      color: isAutoMode ? "#15803d" : "#6b7280",
                      border: "none",
                      borderRadius: "6px",
                      fontSize: "11px",
                      fontWeight: 700,
                ...
                <span style={{ fontSize: "11px", color: "#9ca3af", marginTop: "4px", display: "block" }}>
                  {isAutoMode
                    ? `Will be assigned: ${nextInvoiceNo || "…"}`
                    : "Custom number — must be unique for this invoice type"}
                </span>
              </div>
              <div className="ci-field">
              ...
              >
                <div style={{ borderRight: "1px solid #000", padding: "8px" }}>
                  ...
                  >
                    ...
                    <span>{isAutoMode ? (nextInvoiceNo || "Auto") : (invoiceNo || "---")}</span>
                  </div>
                  <div
                    style={{
                      display: "grid",
