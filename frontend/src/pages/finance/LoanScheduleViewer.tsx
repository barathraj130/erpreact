...
const LoanScheduleViewer: React.FC<{ loanId: number, onClose: () => void }> = ({ loanId, onClose }) => {
    ...
    useEffect(() => {
        const fetchSchedule = async () => {
            try {
                const res = await axios.get(`http://localhost:5001/api/finance/loans/${loanId}/schedule`);
                            ...
                            <tbody>
                                {schedule.map((item, i) => (
                                    <tr key={i}>
                                        ...
                                        <td className="font-bold">₹ {item.total_due.toLocaleString()}</td>
