...
const FinanceDashboard: React.FC = () => {
  ...
  const [financeSummary, setFinanceSummary] = useState({ total_loan_liability: 0, total_chit_commitment: 0 });
       ...
       >
         <div>
           <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
             ...
             {debugInfo && (
               <span style={{ fontSize: "10px", background: "#f1f5f9", padding: "2px 8px", borderRadius: "4px", color: "#64748b", fontWeight: 600 }}>
                 ID: {debugInfo.companyId} | BR: {debugInfo.branchId || 'ALL'}
               </span>
             )}
           </div>
...
<div className="stats-grid" style={{ gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", marginBottom: "32px" }}>
          ...
          <div className="stat-card card-red" style={{ background: 'linear-gradient(135deg, #fff1f2 0%, #ffe4e6 100%)', color: '#9f1239' }}>
            <FaUniversity className="stat-icon" />
            <span className="label">Loan Liability</span>
            <span className="value">₹ {financeSummary.total_loan_liability.toLocaleString()}</span>
            <span className="stat-sub">Principal Outstanding</span>
          </div>
          <div className="stat-card card-indigo" style={{ background: 'linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%)', color: '#5b21b6' }}>
            <FaCoins className="stat-icon" />
            <span className="label">Chit Commitment</span>
            <span className="value">₹ {financeSummary.total_chit_commitment.toLocaleString()}</span>
            <span className="stat-sub">Monthly Installments</span>
          </div>
        </div>
