...
const PayrollRun: React.FC = () => {
  ...
  const fetchPayrollHistory = async () => {
    try {
      const res = await apiFetch(`/hr/payroll/history`);
  ...
  const generatePayroll = async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/hr/payroll/preview`, {
        method: "POST",
      ...
      prev.map((item) => {
        if (item.employee.id === empId) {
          const newDeduction = Math.min(
            Math.max(0, value),
            item.original_deduction,
          );
          return {
            ...item,
            deduction: newDeduction,
            net_pay: item.gross - newDeduction,
          };
        }
        return item;
      }),
  ...
  const processPayroll = async (salaryType: string) => {
    const itemsToProcess = payrollData.filter(
      (p) => !p.skipped && (p.employee.salary_type || "Monthly") === salaryType,
    );
    ...
    if (
      !confirm(
        `Process ${salaryType} payroll for ${itemsToProcess.length} employees?\n\nTotal: ₹${total.toLocaleString()}`,
      )
    )
      return;
    ...
    setProcessing(true);
    try {
      const res = await apiFetch(`/hr/payroll/finalize`, {
        method: "POST",
        body: {
  ...
  const monthlyPayroll = payrollData.filter(
    (p) => (p.employee.salary_type || "Monthly") === "Monthly",
  );
  const weeklyPayroll = payrollData.filter(
    (p) => p.employee.salary_type === "Weekly",
  );
  const dailyPayroll = payrollData.filter(
    (p) => p.employee.salary_type === "Daily",
  );
  ...
  const currentTotalGross = currentActivePayroll.reduce(
    (sum, p) => sum + p.gross,
    0,
  );
  const currentTotalDeductions = currentActivePayroll.reduce(
    (sum, p) => sum + p.deduction,
    0,
  );
  const currentTotalNet = currentActivePayroll.reduce(
    (sum, p) => sum + p.net_pay,
    0,
  );
  ...
  const overallTotalNet = allActivePayroll.reduce(
    (sum, p) => sum + p.net_pay,
    0,
  );
    ...
    <div>
              ...
              <tbody>
                {payrollHistory.slice(0, 15).map((record, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <td style={{ padding: "12px", textAlign: "center" }}>
                      <span
                        style={{
                          ...
                          fontSize: "0.75rem",
                          background:
                            record.salary_type === "Daily"
                              ? "#dcfce7"
                              : record.salary_type === "Weekly"
                                ? "#f5f3ff"
                                : "#e0e7ff",
                          color:
                            record.salary_type === "Daily"
                    ...
                    >
                      ₹{Number(record.total_deductions || 0).toLocaleString()}
                    </td>
                    <td
                      style={{
                        padding: "12px",
      ...
      <div style={{ ...cardStyle, marginBottom: "24px" }}>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <FaCalendarWeek color="#0D9488" size={20} />
              <div>
                <label
                  style={{
                    display: "block",
        ...
        <>
          ...
          >
            ...
            >
              <div>
                ...
                <h1 style={{ margin: "8px 0 0", fontSize: "2.5rem" }}>
                  ₹{overallTotalNet.toLocaleString()}
                </h1>
              </div>
          ...
          >
            <div
              style={{
                ...cardStyle,
                borderLeft: `5px solid ${tabConfig[activeTab].color}`,
                padding: "24px",
              }}
            >
              <div
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    ₹{currentTotalGross.toLocaleString()}
                  </h2>
                </div>
                <div
                  style={{
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    ₹{currentTotalDeductions.toLocaleString()}
                  </h2>
                </div>
                <div
                  style={{
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    ₹{currentTotalNet.toLocaleString()}
                  </h2>
                </div>
                <div
                  style={{
          ...
          <div style={cardStyle}>
            ...
            >
              <h3 style={{ margin: 0, color: "#1e293b" }}>
                {tabConfig[activeTab].label} Payroll
              </h3>
            ...
            >
              <div>
                <p style={{ margin: 0, color: "#64748b" }}>
                  ...
                  <strong>{currentActivePayroll.length}</strong>{" "}
                ...
                >
                  {currentPayroll.filter((p) => p.skipped).length} employees
                  will be skipped
                </p>
              </div>
              <div style={{ display: "flex", gap: "12px" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    background:
                      currentActivePayroll.length > 0
                        ? tabConfig[activeTab].color
                        : "#94a3b8",
                    color: "white",
                    cursor:
                      currentActivePayroll.length > 0
                ...
                >
                  ...
                  {processing
                    ? "Processing..."
                    : `Process ${tabConfig[activeTab].label} (₹${currentTotalNet.toLocaleString()})`}
                </button>
              </div>
            </div>
          )}
      ...
      {payrollData.length === 0 && !loading && (
        <div style={{ ...cardStyle, textAlign: "center", padding: "60px" }}>
          ...
          <h3 style={{ margin: "20px 0 8px", color: "#1e293b" }}>
            Generate Payroll
          </h3>
          <p
            style={{
          ...
          >
            Select a period and click "Generate Payroll" to calculate salaries
            for Monthly, Weekly, and Daily workers
          </p>
        </div>
      )}
    </div>
