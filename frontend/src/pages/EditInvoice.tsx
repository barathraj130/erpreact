...
import { FaArrowLeft, FaCreditCard, FaExchangeAlt, FaGlobe, FaMobileAlt, FaMoneyBillWave, FaPrint, FaReceipt, FaSave, FaTrash, FaUniversity, FaWallet } from "react-icons/fa";
...
const EditInvoice: React.FC = () => {
    ...
    const performConversion = async () => {
        ...
        try {
            const res = await fetch(`https://api.frankfurter.app/latest?amount=${convertAmount}&from=${fromCurrency}&to=INR`);
        ...
        const load = async () => {
            try {
                ...
                setMeta({
                    ...
                    dateOfSupply: inv.date_of_supply ? inv.date_of_supply.substring(0, 10) : new Date().toISOString().slice(0, 10),
    ...
    const rows = items.map(it => {
        ...
        const isSameState = (customer.stateCode || "33") === (company.stateCode || "33");
    ...
    const sectionTitleStyle: React.CSSProperties = { fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-muted)', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' };
    const inputLabelStyle: React.CSSProperties = { fontWeight: 600, fontSize: '0.75rem', color: 'var(--text-main)', marginBottom: '8px', display: 'block' };
    ...
    return (
        <div className="invoice-layout">
            <div className="invoice-form-side">
                <div className="glass-panel" style={{ padding: "16px 24px", borderBottom: "1px solid var(--border-color)", display: "flex", alignItems: "center", gap: "15px", zIndex: 10, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)' }}>
                    <button onClick={() => navigate("/invoices")} className="btn-secondary" style={{ padding: '8px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <FaArrowLeft size={14} />
                    </button>
                    <h2 style={{ fontSize: "1.25rem", fontWeight: 800, flex: 1, margin: 0, color: 'var(--text-main)', letterSpacing: '-0.5px' }}>Edit Invoice #{invoiceNo}</h2>
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button 
                            onClick={() => setShowConverter(!showConverter)} 
                            className="btn-secondary"
                            style={{ 
                                padding: "8px 12px", fontSize: "0.8rem", gap: '6px', display: 'flex', alignItems: 'center', borderColor: showConverter ? 'var(--primary)' : 'var(--border-color)', color: showConverter ? 'var(--primary)' : 'var(--text-main)', background: showConverter ? 'var(--primary-glow)' : 'white'
                            }}
                        >
                            <FaGlobe /> <span style={{ display: window.innerWidth > 1400 ? 'inline' : 'none' }}>Convert</span>
                        </button>
                        <button onClick={printInvoice} className="btn-secondary" style={{ padding: "8px 12px", fontSize: "0.8rem", gap: '6px', display: 'flex', alignItems: 'center' }}><FaPrint /> <span style={{ display: window.innerWidth > 1200 ? 'inline' : 'none' }}>Print</span></button>
                    </div>
                </div>

                {/* Live Currency Converter Tool */}
                {showConverter && (
                    <div className="page-transition" style={{ padding: '20px', background: 'var(--bg-body)', borderBottom: '1px solid var(--border-color)' }}>
                        <div className="card" style={{ border: '1px solid var(--primary-glow)', boxShadow: 'var(--shadow-sm)' }}>
                            <div style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--primary)', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                ...
                                <div style={{ width: '100px' }}>
                                    <select 
                                        ...
                                        style={{ ...inputStyle, background: 'white', appearance: 'auto' }}
                                    >
                            ...
                            {convertedINR !== null && (
                                <div className="page-transition" style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px dashed var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 600 }}>RESULT</span>
                                        <div style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--text-main)' }}>₹ {fmt(convertedINR)}</div>
                                    </div>
                                    <button 
                                        onClick={() => {
                                            const newItems = [...items];
                                            newItems[newItems.length - 1].rate = Math.round(convertedINR * 100) / 100;
                                            setItems(newItems);
                                            setShowConverter(false);
                                        }}
                                        style={{ fontSize: '0.8rem', color: 'var(--primary)', background: 'var(--primary-glow)', border: 'none', fontWeight: 700, cursor: 'pointer', padding: '8px 12px', borderRadius: '6px' }}
                                    >
                                        Apply to Item
                                    </button>
                                </div>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "20px" }}>
                    ...
                    <div style={sectionStyle}>
                        ...
                        {items.map((it, i) => (
                            <div key={i} style={{ background: "#f8fafc", padding: "10px", borderRadius: "6px", border: "1px solid #e2e8f0", marginBottom: "10px", position: "relative" }}>
                                ...
                                <button onClick={() => { const t = [...items]; t.splice(i, 1); setItems(t); }} style={{ position: "absolute", top: "10px", right: "10px", color: "red", border: "none", background: "none", cursor: "pointer" }}><FaTrash /></button>
                            </div>
                        ))}
                    ...
                    <div style={sectionStyle}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "10px", marginBottom: "15px" }}>
                            <div style={{ padding: "10px", background: "#f8fafc", borderRadius: "8px", textAlign: "center" }}><div style={{ fontSize: "0.7rem", color: "#64748b" }}>Total</div><div style={{ fontSize: "1rem", fontWeight: 700 }}>₹{fmt(grandTotal)}</div></div>
                            <div style={{ padding: "10px", background: "#dcfce7", borderRadius: "8px", textAlign: "center" }}><div style={{ fontSize: "0.7rem", color: "#166534" }}>Paid</div><div style={{ fontSize: "1rem", fontWeight: 700, color: "#16a34a" }}>₹{fmt(totalPaid)}</div></div>
                            <div style={{ padding: "10px", background: balanceDue > 0 ? "#fee2e2" : "#dcfce7", borderRadius: "8px", textAlign: "center" }}><div style={{ fontSize: "0.7rem", color: balanceDue > 0 ? "#991b1b" : "#166534" }}>Balance</div><div style={{ fontSize: "1rem", fontWeight: 700, color: balanceDue > 0 ? "#dc2626" : "#16a34a" }}>₹{fmt(Math.max(balanceDue, 0))}</div></div>
                        </div>
                        ...
                        {/* New Payment Entry */}
                         <div style={{ padding: "15px", background: "#f0fdf4", borderRadius: "8px", border: "1px solid #bbf7d0", marginBottom: "15px" }}>
                             <label style={{ ...inputLabelStyle, color: "#166534" }}>Add New Payment</label>
                              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "8px", marginBottom: "8px" }}>
                                 <input type="number" placeholder="Amount" value={payments[0].amount || ""} onChange={e => updatePayment(0, 'amount', Number(e.target.value))} style={inputStyle} />
                                 <select style={inputStyle} value={payments[0].payment_method} onChange={e => updatePayment(0, 'payment_method', e.target.value)}>
                        ...
                        <div style={{ background: "#f8fafc", padding: "10px", borderRadius: "6px", border: "1px dashed #cbd5e1" }}><label style={{ fontSize: "10px", color: "#64748b", fontWeight: "bold", textTransform: "uppercase" }}>Amount in Words</label><div style={{ fontSize: "0.85rem", fontWeight: 600, color: "#1e293b", textTransform: "capitalize" }}>{grandTotal ? toWords(Math.round(grandTotal)) : "-"}</div></div>
                    </div>
                ...
                {/* Footer with Save Button */}
                <div className="glass-panel" style={{ padding: "15px", borderTop: "1px solid var(--border-color)", display: "flex", justifyContent: "space-between", alignItems: "center", background: 'rgba(255,255,255,0.9)', zIndex: 10, flexWrap: "wrap", gap: "16px" }}>
                    <div><div style={{ fontSize: "0.7rem", color: "var(--text-muted)" }}>Grand Total</div><div style={{ fontSize: "1.2rem", fontWeight: "bold", color: "var(--text-main)" }}>₹{fmt(grandTotal)}</div></div>
                    <button className="btn-primary" onClick={handleUpdate} style={{ gap: "8px" }}><FaSave /> Update Invoice</button>
                </div>
            </div>
            ...
            <div className="invoice-preview-side" style={{ overflow: 'hidden' }}>
                <div 
                    ...
                    style={{ 
                        ...
                        transform: windowWidth < 1024 ? `scale(${(windowWidth - 40) / 794})` : 'none',
                        transformOrigin: "top center",
                        marginBottom: windowWidth < 1024 ? `-${297 * 3.78 * (1 - (windowWidth - 40) / 794)}px` : '0',
                        flexShrink: 0
                    }}
                >
                    <div style={{ border: "1px solid #000", width: "100%", position: "relative" }}>
                        {/* TRIPLICATE INDICATOR */}
                        <div style={{ position: "absolute", top: "0", right: "0", fontSize: "10px", textAlign: "right", border: "1px solid #ccc", padding: "4px", zIndex: 10 }}>
                        ...
                        <div style={{ textAlign: "center", padding: "15px 0 0 0", clear: "both" }}>
                            <h1 style={{ margin: "0", fontSize: "32px", fontWeight: "900", textTransform: "uppercase", letterSpacing: "1px" }}>{company.name || "COMPANY NAME"}</h1>
                            <p style={{ margin: "4px 0", fontSize: "11px" }}>
                                {company.address}{company.city_pincode ? `, ${company.city_pincode}` : ''}
                            </p>
                            <p style={{ margin: "3px 0", fontSize: "12px", fontWeight: "bold" }}>{company.state?.toUpperCase() || 'TAMILNADU'}</p>
                            <p style={{ margin: "3px 0 0 0", fontSize: "12px", fontWeight: "bold" }}>GSTIN No. : {company.gstin}</p>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: "1px solid #000", borderBottom: "none", fontSize: "11px" }}>
                            <div style={{ borderRight: "1px solid #000" }}>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", borderBottom: "1px solid #000", padding: "2px 5px" }}>
                                    <span>Reverse Charge</span><span>: {meta.reverseCharge}</span>
                                </div>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", borderBottom: "1px solid #000", padding: "2px 5px" }}>
                                    <span>Invoice No</span><span>: <b>{invoiceNo}</b></span>
                                </div>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", borderBottom: "1px solid #000", padding: "2px 5px" }}>
                                    <span>Invoice Date</span><span>: <b>{new Date(meta.invoiceDate).toLocaleDateString("en-GB")}</b></span>
                                </div>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", padding: "2px 5px" }}>
                                    <span>State</span><span>: {company.state || 'TAMILNADU'} Code: {company.stateCode || '33'}</span>
                                </div>
                            </div>
                            <div>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", borderBottom: "1px solid #000", padding: "2px 5px" }}>
                                    <span>Transport Mode</span><span>: {meta.transportMode}</span>
                                </div>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", borderBottom: "1px solid #000", padding: "2px 5px" }}>
                                    <span>Vehicle Number</span><span>: {meta.vehicleNumber}</span>
                                </div>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", borderBottom: "1px solid #000", padding: "2px 5px" }}>
                                    <span>Date of Supply</span><span>: {new Date(meta.dateOfSupply).toLocaleDateString("en-GB")}</span>
                                </div>
                                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", padding: "2px 5px" }}>
                                    <span>Place of Supply</span><span>: {meta.placeOfSupply || 'TAMILNADU'} Code: {meta.supplyStateCode || '33'}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: "1px solid #000", borderBottom: "none", fontSize: "11px" }}>
                            <div style={{ borderRight: "1px solid #000", padding: "5px" }}>
                                ...
                                <div>State : {customer.state || 'TAMILNADU'} Code : {customer.stateCode || '33'}</div>
                            </div>
                            ...
                            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "9px" }}>
                                <thead>
                                    <tr style={{ borderBottom: "1px solid #000", background: "#f9fafb" }}>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "35px", fontSize: "9px", fontWeight: "bold" }}>Sr.No</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", textAlign: "left", fontSize: "9px", fontWeight: "bold" }}>Name of Product</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "50px", fontSize: "9px", fontWeight: "bold" }}>HSN</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "35px", fontSize: "9px", fontWeight: "bold" }}>Uom</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "35px", fontSize: "9px", fontWeight: "bold" }}>Qty</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "70px", fontSize: "9px", fontWeight: "bold" }}>Rate</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "75px", fontSize: "9px", fontWeight: "bold" }}>Amount</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "60px", fontSize: "9px", fontWeight: "bold" }}>Discount</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "75px", fontSize: "9px", fontWeight: "bold" }}>Taxable</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "55px", fontSize: "9px", fontWeight: "bold" }}>CGST</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "55px", fontSize: "9px", fontWeight: "bold" }}>SGST</th>
                                        <th style={{ borderRight: "1px solid #000", padding: "4px 2px", width: "55px", fontSize: "9px", fontWeight: "bold" }}>IGST</th>
                                        <th style={{ padding: "4px 2px", width: "85px", fontSize: "9px", fontWeight: "bold" }}>Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {Array.from({ length: 15 }).map((_, i) => {
                                        ...
                                        return (
                                            <tr key={i}>
                                                ...
                                                <td style={tableCellStyle}>{r.rate ? fmt(r.rate) : ""}</td>
                                                <td style={tableCellStyle}>{r.amount ? fmt(r.amount) : ""}</td>
                                                <td style={tableCellStyle}>0.00</td>
                                                <td style={tableCellStyle}>{r.taxable ? fmt(r.taxable) : ""}</td>
                                                <td style={tableCellStyle}>{r.cgst > 0 ? fmt(r.cgst) : ""}</td>
                                                <td style={tableCellStyle}>{r.sgst > 0 ? fmt(r.sgst) : ""}</td>
                                                <td style={tableCellStyle}>{r.igst > 0 ? fmt(r.igst) : ""}</td>
                                                <td style={tableCellStyle}>{r.lineTotal ? fmt(r.lineTotal) : ""}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", border: "1px solid #000", borderTop: "none", fontSize: "11px" }}>
                            ...
                            <div style={{ padding: "0" }}>
                                <table style={{ width: "100%", fontSize: "11px", borderCollapse: "collapse" }}>
                                    <tbody>
                                        <tr><td style={{ borderBottom: '1px solid #000', padding: '4px' }}>Total Before Tax</td><td style={{ borderBottom: '1px solid #000', padding: '4px', textAlign: 'right', borderLeft: "1px solid #000", width: "100px" }}><b>{fmt(totalTaxable)}</b></td></tr>
                                        <tr><td style={{ borderBottom: '1px solid #000', padding: '4px' }}>Add: CGST</td><td style={{ borderBottom: '1px solid #000', padding: '4px', textAlign: 'right', borderLeft: "1px solid #000" }}>{fmt(totalCGST)}</td></tr>
                                        <tr><td style={{ borderBottom: '1px solid #000', padding: '4px' }}>Add: SGST</td><td style={{ borderBottom: '1px solid #000', padding: '4px', textAlign: 'right', borderLeft: "1px solid #000" }}>{fmt(totalSGST)}</td></tr>
                                        <tr><td style={{ borderBottom: '1px solid #000', padding: '4px' }}>Add: IGST</td><td style={{ borderBottom: '1px solid #000', padding: '4px', textAlign: 'right', borderLeft: "1px solid #000" }}>{fmt(totalIGST)}</td></tr>
                                        <tr><td style={{ borderBottom: '1px solid #000', padding: '4px' }}>Tax Amount: GST</td><td style={{ borderBottom: '1px solid #000', padding: '4px', textAlign: 'right', borderLeft: "1px solid #000" }}><b>{fmt(totalGST)}</b></td></tr>
                                        <tr style={{ background: '#f3f4f6', fontSize: "12px" }}><td style={{ padding: '6px' }}><b>Total After Tax</b></td><td style={{ padding: '6px', textAlign: 'right', borderLeft: "1px solid #000" }}><b>{fmt(grandTotal)}</b></td></tr>
                                    </tbody>
                                </table>
                                <div style={{ padding: '6px', fontSize: '10px', borderTop: "1px solid #000" }}>GST on Reverse Charge : {meta.reverseCharge}</div>
                                ...
                                <div style={{ textAlign: "right", fontWeight: "bold", marginTop: "10px", paddingRight: "10px", fontSize: "11px" }}>
                                    ...
                                    <div style={{ height: "45px", display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                                        {(company.signature_url || user?.signature_url) && (
                                            <img src={company.signature_url || user?.signature_url} alt="Signature" style={{ height: "35px" }} />
                                        )}
                                    </div>
