    ...
    const load = async () => {
      try {
        const [profile, invoiceRes] = await Promise.all([
          fetchProfile().catch(() => ({})),
          apiFetch(`/invoice/${id}`).catch(() => null),
        ]);

        const cp = profile as any;
        if (cp && !cp.error && cp.company_name) {
          setCompany({
            name: cp.company_name || "COMPANY NAME",
            address: cp.address_line1 || "COMPANY ADDRESS",
            city_pincode: cp.city_pincode || "",
            state: cp.state || "TAMILNADU",
            stateCode: cp.state_code || "33",
            gstin: cp.gstin || "---",
            bank_name: cp.bank_name || "",
            ac_no: cp.bank_account_no || "",
            ifsc: cp.bank_ifsc_code || "",
            signature_url: cp.signature_url || null,
          });
        }
