...
const Marketplace: React.FC = () => {
    ...
    if (loading) return <div style={{textAlign: 'center', padding: '50px', color: '#64748b'}}>Loading Catalog...</div>;

    return (
        <div>
            <div style={{ marginBottom: '30px' }}>
                <h1 style={{ fontSize: '2rem', fontWeight: 700, color: '#1e293b', marginBottom: '10px' }}>Product Catalog</h1>
                <p style={{ color: '#64748b' }}>Browse our latest inventory and place orders directly.</p>
            </div>
            ...
            {products.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px', background: 'white', borderRadius: '12px', border: '1px dashed #e2e8f0' }}>
                    ...
                    <p style={{ color: '#94a3b8' }}>No active products available at the moment.</p>
                </div>
            ) : (
                        ...
                        <div key={product.id} style={{ 
                            ...
                            <div style={{ height: '200px', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#cbd5e1', position: 'relative' }}>
                                ...
                                {product.current_stock > 0 ? (
                                    <span style={{ position: 'absolute', top: 10, right: 10, background: '#dcfce7', color: '#166534', fontSize: '0.75rem', padding: '4px 8px', borderRadius: '12px', fontWeight: 600 }}>
                                        In Stock
                                    </span>
                                ) : (
                                    <span style={{ position: 'absolute', top: 10, right: 10, background: '#fee2e2', color: '#991b1b', fontSize: '0.75rem', padding: '4px 8px', borderRadius: '12px', fontWeight: 600 }}>
                                        Out of Stock
                                    </span>
                                )}
                            </div>
                            ...
                            <div style={{ padding: '20px', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #f1f5f9', paddingTop: '15px' }}>
                                    ...
                                    <button 
                                        onClick={() => alert("Order placement will be enabled soon. Please contact admin for manual orders.")}
                                        style={{ 
                                            background: '#2563eb', color: 'white', border: 'none', 
                                            padding: '10px 16px', borderRadius: '8px', cursor: 'pointer',
                                            fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px',
                                            transition: 'background 0.2s', opacity: product.current_stock > 0 ? 1 : 0.5
                                        }}
