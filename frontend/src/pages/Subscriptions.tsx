        ...
        if (companyId) {
          ...
          if (companyRes.ok && companyData.subscription_id) {
            const plan = plansData.find(
              (p: Plan) => p.id === companyData.subscription_id,
            );
            setMyPlan(plan);
          }
        }
    ...
    <div className="space-y-10">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-4">
        ...
        <p className="text-slate-500 text-lg">
          Scale your enterprise with precision-engineered modules and flexible
          resource limits.
        </p>
      </div>
          ...
          <div className="bg-white rounded-[22px] p-8 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-xs font-black tracking-widest uppercase">
                  Your Current Plan
                </span>
              ...
              <div className="flex gap-6 items-center text-slate-500 font-medium">
                <span className="flex items-center gap-2">
                  <FaLayerGroup /> {myPlan.max_branches} Branches
                </span>
                <span className="flex items-center gap-2">
                  <FaCheckCircle /> {myPlan.max_users} Users
                </span>
                <span className="flex items-center gap-2 font-bold text-slate-900">
                  Expires:{" "}
                  {new Date(myPlan.expiry_date || "").toLocaleDateString()}
                </span>
              </div>
            </div>
            <button className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-slate-200  transition-all text-sm uppercase tracking-widest">
      ...
      >
        {plans.length > 0
          ? plans.map((plan, idx) => (
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div
                    className={`p-4 rounded-2xl ${idx === 0 ? "bg-slate-50 text-slate-400" : idx === 1 ? "bg-blue-50 text-blue-600" : "bg-amber-50 text-amber-600"}`}
                  >
                    {idx === 0 ? (
                      <FaRocket size={24} />
                    ) : idx === 1 ? (
                ...
                <div className="space-y-4 mb-10">
                  <FeatureItem
                    ...
                    text={`${plan.max_branches} Physical Branches`}
                  />
                  <FeatureItem
                    icon={<FaCheckCircle className="text-emerald-500" />}
                    text={`${plan.max_users} Total Users`}
                  />
                  <FeatureItem
                    icon={<FaCheckCircle className="text-emerald-500" />}
                    text={`${plan.storage_limit_gb}GB Enterprise Storage`}
                  />
                ...
                <button
                  className={`w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all ${plan.id === myPlan?.id ? "bg-slate-100 text-slate-400 cursor-default" : "bg-blue-600 text-white shadow-lg shadow-blue-200 hover:bg-blue-700  "}`}
                >
          ...
          : [1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-[500px] bg-slate-50 animate-pulse rounded-3xl"
              />
            ))}
      </motion.div>
      ...
      <div className="bg-slate-900 text-white p-12 rounded-[40px] flex flex-col items-center text-center gap-6">
        <h3 className="text-2xl font-bold italic tracking-tighter">
          Need a custom solution for your enterprise?
        </h3>
        <p className="text-slate-400 max-w-md">
          Our global account managers can design a personalized blueprint for
          your multi-country branch operations.
        </p>
        <button className="bg-white text-slate-900 px-8 py-3 rounded-full font-bold  transition-all">
