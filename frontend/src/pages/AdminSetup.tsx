...
const AdminSetup: React.FC = () => {
  ...
  const handleSaveBank = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const isEdit = !!bankToEdit;
      const url = isEdit ? `/company/bank-accounts/${bankToEdit?.id}` : "/company/bank-accounts";
      const method = isEdit ? "PUT" : "POST";
      ...
      alert("Network error saving bank account");
    }
  };

  const openBankModal = (bank: any | null = null) => {
    if (bank) {
      setBankToEdit(bank);
      setBankForm(bank);
    } else {
  ...
  const deleteBank = async (id: number) => {
    ...
    try {
      const res = await apiFetch(`/company/bank-accounts/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
      else {
              ...
              <div className="page-table-wrapper" style={{ minHeight: "400px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ padding: "32px", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, 1fr)", gap: "24px" }}>
                      {[
                        ...
                        { label: "PAN Number", value: profile?.pan_number || "Not provided" },
                        { label: "Company Type", value: profile?.company_type || "Proprietorship" },
                        { label: "FY Start", value: profile?.financial_year_start_month ? `Month ${profile.financial_year_start_month}` : "April" },
                        { label: "Currency", value: profile?.default_currency || "INR" },
                ...
                {activeItem === "BILLING" && (
                  <div>
                    <div style={{ padding: "24px 32px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        <div style={{ fontSize: "16px", fontWeight: 600 }}>Finance & Banking</div>
                        <div style={{ fontSize: "12.5px", color: "var(--text-3)", marginTop: "2px" }}>Corporate bank accounts and settlement configurations.</div>
                      </div>
                      <button className="page-btn-round page-btn-round-primary" onClick={() => openBankModal()}>
                        ...
                        {bankAccounts.map(b => (
                          <tr key={b.id}>
                            <td>
                              ...
                              <div className="font-mono text-sm">{b.account_number}</div>
                            </td>
                            <td><span className="type-badge type-badge-slate" style={{ fontSize: '10px' }}>{b.purpose || 'General'}</span></td>
                            <td>
                              {b.is_default && <span className="type-badge type-badge-green" style={{ fontSize: '10px' }}>Default</span>}
                            </td>
                            <td className="text-right">
                              <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                <button className="page-btn-round-sm" onClick={() => openBankModal(b)}><FaEdit size={12} /></button>
                                <button className="page-btn-round-sm" style={{ color: 'var(--red)' }} onClick={() => deleteBank(b.id)}><FaArrowLeft size={12} style={{ transform: 'rotate(-45deg)' }} /></button>
                              </div>
                            </td>
                          </tr>
                        ))}
              ...
              <form onSubmit={handleUpdateProfile}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <div>
                    ...
                    <select 
                      value={profileForm.company_type || "Proprietorship"}
                  ...
                  <div>
                    ...
                    <input 
                      value={profileForm.default_currency || "INR"}
                      onChange={e => setProfileForm({...profileForm, default_currency: e.target.value})}
                    />
                  </div>
                </div>
                <label>Email</label>
              ...
              <form onSubmit={handleSaveBranch}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <div>
                    <label>City & Pin</label>
                    <input 
                      value={branchForm.city_pincode || ""}
                      onChange={e => setBranchForm({...branchForm, city_pincode: e.target.value})}
                    />
                  </div>
                  <div>
        ...
        {showBankModal && (
          <div className="page-modal-overlay">
            <motion.div 
              ...
              exit={{ scale: 0.95, opacity: 0 }}
            >
              <h2>{bankToEdit ? "Edit Bank Account" : "Add Bank Account"}</h2>
              <form onSubmit={handleSaveBank}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  ...
                  <div>
                    ...
                    <input 
                      type="number"
                      value={bankForm.opening_balance || 0}
                ...
                <select 
                  value={bankForm.purpose || "DEFAULT"}
                  onChange={e => setBankForm({...bankForm, purpose: e.target.value})}
                >
                  <option value="DEFAULT">General Operations</option>
                  <option value="SALARY">Payroll Account</option>
                  <option value="PURCHASE">Purchase Account</option>
                  <option value="CASH">Cash in Hand</option>
                </select>
                <div style={{ display: "flex", gap: "10px", alignItems: "center", marginTop: "16px", marginBottom: "20px" }}>
                   <input 
                     type="checkbox" 
                     ...
                     style={{ width: "auto", margin: 0 }}
                   />
                   <span style={{ fontSize: "13px", fontWeight: 600 }}>Set as primary default account</span>
                </div>
                <div className="page-modal-actions">
                  <button type="button" className="page-btn-round" onClick={() => setShowBankModal(false)} style={{ flex: 1 }}>Cancel</button>
                  <button type="submit" className="page-btn-round page-btn-round-primary" style={{ flex: 1 }}>Save Account</button>
                </div>
              </form>
            </motion.div>
          </div>
