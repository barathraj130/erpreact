...
const InvoiceDetails: React.FC = () => {
  ...
  const rows = items.map((item: any) => {
    ...
    const gstRate = val(item.tax_percent || item.gst_rate || 18);
    const gstAmount = (taxable * gstRate) / 100;

    // Use stored components if available, otherwise calculate
    const cgst = val(item.cgst_amount) || (isSameState ? gstAmount / 2 : 0);
    const sgst = val(item.sgst_amount) || (isSameState ? gstAmount / 2 : 0);
    const igst = val(item.igst_amount) || (!isSameState ? gstAmount : 0);
                ...
                <tfoot>
                    ...
                    >
                        ...
                        {isSameState ? (
                          <>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                              <span>Add: CGST @ {fmt(val(data.cgst_total) / val(data.sub_total) * 100 / 2 || 9)}% on {fmt(totalTaxable)}</span>
                              <span>{fmt(totalCGST)}</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #ccc', paddingBottom: '2px' }}>
                              <span>Add: SGST @ {fmt(val(data.sgst_total) / val(data.sub_total) * 100 / 2 || 9)}% on {fmt(totalTaxable)}</span>
                              <span>{fmt(totalSGST)}</span>
                            </div>
                          </>
                        ) : (
                          <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #ccc', paddingBottom: '2px' }}>
                            <span>Add: IGST @ {fmt(val(data.igst_total) / val(data.sub_total) * 100 || 18)}% on {fmt(totalTaxable)}</span>
                      ...
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: "10px", color: "#666" }}>Sub Total: ₹{fmt(totalTaxable)}</div>
                        <div style={{ fontSize: "14px", fontWeight: "900", marginTop: "4px" }}>
