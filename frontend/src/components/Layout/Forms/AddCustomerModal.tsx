...
import { FaBuilding, FaEnvelope, FaIdCard, FaLock, FaMapMarkerAlt, FaMoneyCheckAlt, FaPhone, FaSave, FaTag, FaTimes, FaUser } from 'react-icons/fa';
...
const styles = {
    ...
    body: { 
        ...
        flexGrow: 1, // Allow body to take available space
        maxHeight: 'calc(90vh - 140px)' // Ensure space for header/footer
    },
    footer: { 
...
const InputField = ({ label, icon: Icon, value, onChange, placeholder, required = false, type = "text", disabled = false }: any) => (
    <div style={styles.inputGroup}>
        ...
        <div style={styles.inputWrapper}>
            ...
            <input type={type} required={required} disabled={disabled} style={{...styles.input, opacity: disabled ? 0.7 : 1, backgroundColor: disabled ? '#f1f5f9' : '#fff'}} placeholder={placeholder} value={value} onChange={onChange} />
        </div>
    </div>
);
...
const AddCustomerModal: React.FC<Props> = ({ onClose, onSuccess, customerToEdit }) => {
    ...
    const handlePhoneChange = (e: any) => {
        ...
        setFormData(prev => ({
            ...prev, 
            phone: val,
            password: prev.password ? prev.password : val 
        }));
    };
            ...
            <div style={styles.modal}>
                ...
                <div style={styles.body}>
                    <form id="customer-form" onSubmit={handleSubmit}>
                        ...
                        {/* 2. PORTAL LOGIN (Moved to Top) */}
                        <div style={{background: '#f0fdf4', padding: '15px', borderRadius: '8px', border: '1px solid #bbf7d0', marginBottom: '20px', marginTop: '10px'}}>
                            ...
                            <div style={styles.row}>
                                <InputField label="Login Username" icon={FaUser} value={formData.username} disabled={true} placeholder="Auto-filled" />
                                <InputField 
                                    ...
                                    placeholder={formData.phone || "Enter password"} 
                                />
                            </div>
                            <p style={{fontSize: '0.75rem', color: '#166534', margin: 0, marginTop: '5px'}}>
                                * The customer can login using these details to view orders and ledger.
                            </p>
                        </div>
                        ...
                        <div style={styles.row}>
                            ...
                            <InputField label="Phone (Default Password)" icon={FaPhone} value={formData.phone} onChange={handlePhoneChange} placeholder="98765..." />
                        </div>
                        <div style={styles.row}>
                        ...
                        <InputField label="Bank Name" icon={FaMoneyCheckAlt} value={formData.bank_name} onChange={(e: any) => setFormData({...formData, bank_name: e.target.value})} placeholder="e.g. HDFC Bank" />
                        <div style={styles.row}>
                            ...
                            <InputField label="IFSC Code" icon={FaTag} value={formData.bank_ifsc_code} onChange={(e: any) => setFormData({...formData, bank_ifsc_code: e.target.value})} placeholder="HDFC000123" />
                        </div>
