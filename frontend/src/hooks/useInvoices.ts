...
export const useInvoices = (): UseInvoicesState => {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const sortedData = data.sort((a, b) => new Date(b.invoice_date).getTime() - new Date(a.invoice_date).getTime());
            setInvoices(sortedData);
        } catch (err) {
            console.error("Failed to load invoices:", err);
            setError(err instanceof Error ? err.message : "Error fetching invoice data.");
            setInvoices([]);
        } finally {
            setLoading(false);
        }
