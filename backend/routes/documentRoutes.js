...
const groupDocumentsByMonth = (rows, isSales) => {
    ...
    rows.forEach(row => {
        ...
        if (isSales) {
            ...
            isTaxDoc = (cat === 'TAX_INVOICE' || cat === 'NOMINAL_TAX_INVOICE');
        } else {
        ...
        const docObj = {
            ...
            type: row.category // original type for detail view
        };
...
router.get("/view/:viewerId", async (req, res) => {
    try {
        ...
        // NO FILE UPLOADED? Generate a dynamic dummy bill
        if (branch === "Sales") {
            return res.redirect(`/api/invoice/${fileId}/pdf`);
        } else {
            return res.redirect(`/api/purchase-pdf/${fileId}`);
        }
    } catch (err) {
