...
router.get('/consolidated/financials', authMiddleware, async (req, res) => {
    ...
    try {
        // 💰 REVENUE COLLATION
        let revenueSql = 'SELECT SUM(total_amount) as total FROM invoices WHERE company_id = $1';
        let expenseSql = 'SELECT SUM(total_amount) as total FROM purchase_bills WHERE company_id = $1';
        const params = [companyId];

        if (branch_id) {
            revenueSql += ' AND branch_id = $2';
            expenseSql += ' AND branch_id = $2';
            params.push(branch_id);
        }

        const [revenue, expense] = await Promise.all([
            db.pgGet(revenueSql, params),
            db.pgGet(expenseSql, params)
        ]);
        ...
        res.json({
            ...
            financials: {
                ...
                net_profit: parseFloat(revenue?.total || 0) - parseFloat(expense?.total || 0)
            },
            inventory: {
